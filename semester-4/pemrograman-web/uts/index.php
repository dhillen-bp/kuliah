...
<head>
    <title>CRUD Dalam Satu File</title>
    <style>
        h2 {
            color: green;
        }
...
<body>
    ...
    <?php
    // --- koneksi ke database
    $koneksi = mysqli_connect("localhost", "root", "", "pertanian_dhillen") or die(mysqli_error($koneksi));
    ...
    function tambah($koneksi)
    {
        ...
        if (isset($_POST['btn_simpan'])) {
            ...
            if (!empty($nm_tanaman) && !empty($hasil) && !empty($lama) && !empty($tgl_panen) && !empty($kd_kategori)) {
                $sql = "INSERT INTO tabel_panen179 (id,nama_tanaman, hasil_panen, lama_tanam, tanggal_panen,kode_kategori) VALUES (" . $id . ",'" . $nm_tanaman . "','" . $hasil . "','" . $lama . "','" . $tgl_panen . "','" . $kd_kategori . "')";
        ...
        <form action="" method="POST">
            <fieldset>
                ...
                <select name="kategori" id="">
                    ...
                    $query = mysqli_query($koneksi, "SELECT * FROM kategori_179 ORDER BY id_kategori");
                ...
                <h3><a href="kategori.php">Data Kategori</a></h3>
            </fieldset>
        </form>
        <?php
    }
    ...
    {
        // $sql = "SELECT * FROM tabel_panen179";
        $join = "SELECT * FROM kategori_179 JOIN tabel_panen179 ON kategori_179.kode_kategori = tabel_panen179.kode_kategori";
        ...
        ?>
            <tr>
                ...
                <td>
                    <a href="index.php?aksi=update&id=<?php echo $data['id']; ?>&kategori=<?php echo $data['nama_kategori']; ?>&nama=<?php echo $data['nama_tanaman']; ?>&hasil=<?php echo $data['hasil_panen']; ?>&lama=<?php echo $data['lama_tanam']; ?>&tanggal=<?php echo $data['tanggal_panen']; ?>">Ubah</a> |
    ...
    function ubah($koneksi)
    {
        ...
        if (isset($_POST['btn_ubah'])) {
            ...
            if (!empty($nm_tanaman) && !empty($hasil) && !empty($lama) && !empty($tgl_panen)) {
                $perubahan = "nama_tanaman='" . $nm_tanaman . "',hasil_panen=" . $hasil . ",lama_tanam=" . $lama . ",tanggal_panen='" . $tgl_panen . "'" . ",kode_kategori='" . $kd_kategori . "'";
                $sql_update = "UPDATE tabel_panen179 SET " . $perubahan . " WHERE id=$id";
                ...
                <fieldset>
                    ...
                    <select name="kategori" id="kategori">
                        ...
                        while ($data = mysqli_fetch_array($query)) {
                            ...
                            echo "<option $select value=$data[kode_kategori]>$data[nama_kategori]</option>";
                        }
                        ?>
                    </select><br>
    ...
    function hapus($koneksi)
    {
        if (isset($_GET['id']) && isset($_GET['aksi'])) {
            $id = $_GET['id'];
            $sql_hapus = "DELETE FROM tabel_panen179 WHERE id=" . $id;
